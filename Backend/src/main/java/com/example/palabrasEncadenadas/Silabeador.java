...
public class Silabeador {
	...
	public ArrayList<String> silabear(String w) {
		...
		// Caso especial por extranjerismo
		if (w.equals("geisha")) {
			silaba = "gei";
			silabas.add(silaba);
			silaba = "sha";
	...
	public int next_s(String w) {
		...
		// sabemos que todas las letras anteriores a vocal + vocal forman parte de la
		// sílaba, veamos las siguientes
		...
		if (esVocal(a[l1]) && esConsonante(a[l2])) {
			if (a[l2] == 'h') {
				return a[l2];
			}
		}
	...
	public int tonica(List<String> silabas) {
		...
		if (patron_vocal_n_s.matcher(ultima).matches())
			return silabas.size() - 2; // llana
		else
			return silabas.size() - 1; // aguda
	}
